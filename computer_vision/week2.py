...
print(
    "Depth value of the very bottom-right pixel of depth map {0} is {1:0.3f}".format(
        i, depth_val
    )
)
...
print(
    "Coordinates of the first keypoint in frame {0}: {1}\n".format(
        i, str(kp_list[i][0].pt)
    )
)
...
print(
    "Number of features matched in frames {0} and {1}: {2}".format(i, i + 1, len(match))
)
...
def filter_matches_distance(match, dist_threshold):
    ...
    ### START CODE HERE ###
    #     filtered_match = [[0,0] for i in xrange(len(match))]
...
print(
    "Number of features matched in frames {0} and {1} after filtering by distance: {2}".format(
        i, i + 1, len(filtered_match)
    )
)
...
i = 0
print(
    "Number of features matched in frames {0} and {1}: {2}".format(
        i, i + 1, len(matches[i])
    )
)
...
if len(filtered_matches) > 0:
    ...
    i = 0
    print(
        "Number of filtered matches in frames {0} and {1}: {2}".format(
            i, i + 1, len(filtered_matches[i])
        )
    )
...
image_move = visualize_camera_movement(
    image1, image1_points, image2, image2_points, is_show_img_after_move=True
)
plt.figure(figsize=(16, 12), dpi=100)
plt.imshow(image_move)
